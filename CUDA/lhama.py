...
# -*- coding: utf-8 -*-
"""
Treino de um Transformer estilo LLaMA (puro, não‑MoE) com:
- RMSNorm
- Atenção multi‑cabeças com RoPE (Rotary Positional Embeddings)
- MLP SwiGLU
- Carregamento direto de dados pré-tokenizados
- Mixed precision (bf16/amp)
- Gradiente acumulado, clipping e warmup+cosine scheduler
- GRUPOS DE WEIGHT DECAY CORRETOS
- LÓGICA DE GRADSCALER E AUTOCAST CORRIGIDA
- INICIALIZAÇÃO DE PESOS GARANTIDA
- CHECKPOINTING E RETOMADA
- FLASH-ATTENTION COM DTYPE FORÇADO PARA BF16
"""
...
# Dependência do `datasets` é necessária para carregar os arquivos .arrow
try:
    ...
    print("⚠️  A biblioteca `datasets` não foi encontrada. Instale com: pip install datasets")
...
# ------------------------------------------------------------
# Utilidades de carregamento de dados
# ------------------------------------------------------------
...
class PretokenizedArrowDataset(Dataset):
    def __init__(self, dataset_folder):
        if not DATASETS_AVAILABLE:
            raise ImportError("A biblioteca `datasets` é necessária. Instale com: pip install datasets")
        
        arrow_files = glob.glob(os.path.join(dataset_folder, "*.arrow"))
        if not arrow_files:
            raise ValueError(f"Nenhum arquivo .arrow encontrado em {dataset_folder}")
        
        datasets = [HFDataset.from_file(file) for file in arrow_files]
        self.dataset = concatenate_datasets(datasets)
        print(f"✅ Dataset pré-fatiado carregado: {len(self.dataset):,} amostras de {len(arrow_files)} arquivos .arrow")
...
def wd_groups(model: nn.Module, weight_decay: float) -> List[Dict]:
    """Separa os parâmetros do modelo em dois grupos: um com e outro sem weight decay."""
    decay, no_decay = [], []
...
def print_model_info(model: LLaMAModel, cfg: TrainConfig, ds_len: int):
    ...
    print("="*60)
    print(f" 🏛️   Arquitetura:")
    print(f" │  ├─ Estilo: ................. LLaMA (Denso)")
    print(f" │  ├─ Dimensão do Modelo (d_model): {model.d_model}")
    print(f" │  ├─ Número de Camadas (n_layers): {model.n_layers}")
    print(f" │  ├─ Número de Cabeças (n_heads): .. {model.n_heads}")
    print(f" │  └─ Tamanho do Vocabulário: ..... {model.vocab_size:,}")
    print(f" ----------------------------------------------------------")
    print(f" 🧮   Parâmetros:")
    print(f" │  ├─ Total: .................... {total_params/1e6:.2f}M ({total_params:,})")
    print(f" │  └─ Treináveis: .............. {trainable_params/1e6:.2f}M ({trainable_params:,})")
    print(f" ----------------------------------------------------------")
    print(f" 🚀   Configuração de Treino:")
    print(f" │  ├─ Comprimento da Sequência: ... {cfg.seq_len}")
    print(f" │  ├─ Total de Amostras: ......... {ds_len:,}")
    print(f" │  ├─ Batch Size por GPU: ........ {cfg.batch_size}")
    print(f" │  ├─ Acumulação de Gradiente: ... {cfg.grad_accum}")
    print(f" │  └─ Batch Size Efetivo: ........ {cfg.batch_size * cfg.grad_accum}")
    print("="*60)
...
def load_checkpoint(model: nn.Module, opt: torch.optim.Optimizer, path: str) -> int:
    ...
    print(f"📂 Retomado de: {path} (step={step})")
    return step
...
def train(cfg: TrainConfig):
    ...
    print(f"\n🚀 INICIANDO TREINO para {total_steps} passos...")
    ...
    elapsed = time.time() - t0
    print("\n✅ Treino concluído!")
    print(f"⏱️ Tempo total: {elapsed/3600:.2f}h | Tokens/s (média): {tokens_seen/max(1e-9, elapsed):,.0f}")
...
def parse_args() -> TrainConfig:
    ...
    p.add_argument('--resume', type=str, default=None, help="Caminho para o checkpoint a ser retomado")
    args = p.parse_args()
